...
// ── CORS Origins ───────────────────────────────────────────────────────────────
// In production, Render sets CORS_ORIGINS env var.
...
app.use((req, res, next) => {
  ...
  // Allow requests from any listed origin OR Capacitor (capacitor://) OR no origin (mobile)
...
app.post('/api/orders', (req, res) => {
  ...
  const order = {
    ...
    description: description || '', budget: Number(budget), deadline,
    quantity: Number(quantity) || 1, category: category || '',
    specifications: specifications || '', status: 'available',
...
app.post('/api/orders/labour', (req, res) => {
  ...
  if (!title || !budget || !deadline) return res.status(400).json({ error: 'Title, budget aur deadline zaroori hain' });
  const order = {
    _id: Date.now().toString(), type: 'labour', title,
    description: description || '', budget: Number(budget), deadline,
    quantity: Number(quantity) || 1, status: 'open', applicants: [],
    hiredLabour: null, postedBy: postedBy || null, createdAt: new Date().toISOString(),
  };
  orders.push(order); saveOrders(orders);
  res.json(order);
});
...
// Frontend URL used for Stripe redirect — read from env, no hardcoded IP
...
app.post('/api/escrow/stripe/initiate', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // ✅ Uses FRONTEND_URL env var instead of hardcoded IP
...
// ── Dispute Routes ─────────────────────────────────────────────────────────────
app.use('/api/dispute', require('./routes/disputeRoutes'));
...
// ── Health check ───────────────────────────────────────────────────────────────
app.get('/health', (req, res) => res.json({ status: 'ok', service: 'auth-server', env: process.env.NODE_ENV }));
...
// ── Notification helper (used by order apply routes) ──────────────────────────
async function createNotification({ title, message, type, userId, meta }) {
  try {
    const Notification = require('./models/Notification');
    await Notification.create({ title, message, type, userId: userId ? String(userId) : null, meta });
  } catch (e) {
...
httpServer.listen(PORT, '0.0.0.0', () =>
  console.log(`✅ Auth server running on port ${PORT} [${process.env.NODE_ENV || 'development'}]`)
);
